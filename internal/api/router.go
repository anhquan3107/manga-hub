...
func NewRouter(
	...
	{
		mangaProtected.POST("", func(c *gin.Context) {
			...
			item, err := mangaService.Create(c.Request.Context(), req)
			if err != nil {
		...
		mangaProtected.PUT("/:id", func(c *gin.Context) {
			...
			item, err := mangaService.Update(c.Request.Context(), c.Param("id"), req)
			if err != nil {
				if isNotFound(err) {
					utils.Error(c, http.StatusNotFound, "manga not found")
					return
		...
		mangaProtected.DELETE("/:id", func(c *gin.Context) {
			err := mangaService.Delete(c.Request.Context(), c.Param("id"))
			if err != nil {
				if isNotFound(err) {
					utils.Error(c, http.StatusNotFound, "manga not found")
					return
